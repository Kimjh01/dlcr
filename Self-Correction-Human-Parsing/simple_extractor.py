...
def main():
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(dataloader)):
            ...
            for i, row in enumerate(parsing_result):
                for j, pixel in enumerate(row):
                        if pixel not in [0, 1, 2, 13, 14, 15, 16, 17]:
                            parsing_result[i][j] = 1
                        else:
                            parsing_result[i][j] = 0
            os.makedirs(os.path.join(args.output_dir, img_folder), exist_ok=True)
            parsing_result_path = os.path.join(args.output_dir, img_folder, img_name[:-4] + '.png')
            output_img = Image.fromarray(np.asarray(parsing_result*255, dtype=np.uint8))
