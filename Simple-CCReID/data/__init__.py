...
from data.dataloader_prcc import ImageDatasetPrcc
...
def build_dataloader(config):
    ...
    # image dataset
        ...
        dataset_builder = ImageDataset
        if config.DATA.DATASET == 'prcc' and not config.EVAL_MODE:
            dataset_builder = ImageDatasetPrcc
        train_sampler = DistributedRandomIdentitySampler(dataset.train, 
                                                         num_instances=config.DATA.NUM_INSTANCES, 
                                                         seed=config.SEED)
        trainloader = DataLoaderX(dataset=dataset_builder(dataset.train, config=config, transform=transform_train),
                                 sampler=train_sampler,
                                 batch_size=config.DATA.TRAIN_BATCH, num_workers=config.DATA.NUM_WORKERS,
                                 pin_memory=True, drop_last=True)

        galleryloader = DataLoaderX(dataset=dataset_builder(dataset.gallery, config=config, transform=transform_test),
                                   sampler=DistributedInferenceSampler(dataset.gallery),
                                   batch_size=config.DATA.TEST_BATCH, num_workers=config.DATA.NUM_WORKERS,
                                   pin_memory=True, drop_last=False, shuffle=False)

        if config.DATA.DATASET == 'prcc':
            queryloader_same = DataLoaderX(dataset=dataset_builder(dataset.query_same,config=config, transform=transform_test),
                                     sampler=DistributedInferenceSampler(dataset.query_same),
                                     batch_size=config.DATA.TEST_BATCH, num_workers=config.DATA.NUM_WORKERS,
                                     pin_memory=True, drop_last=False, shuffle=False)
            queryloader_diff = DataLoaderX(dataset=dataset_builder(dataset.query_diff, config=config, transform=transform_test),
            ...
            queryloader = DataLoaderX(dataset=dataset_builder(dataset.query, transform=transform_test),
