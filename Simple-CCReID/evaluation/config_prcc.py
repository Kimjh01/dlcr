...
_C.DATA.ROOT = ''
_C.DATA.ROOT_GALLERY = _C.DATA.ROOT+'/prcc/rgb/test'
_C.DATA.ROOT_QUERY = 'generated data'

# Dataset for evaluation
_C.DATA.DATASET = 'prcc'
# Workers for dataloader
_C.DATA.NUM_WORKERS = 8
_C.DATA.QUERY_NUM_WORKERS = 1
...
_C.DATA.MAX_BATCH_SIZE=64
# Batch size for testing
_C.DATA.TEST_BATCH = 1
_C.DATA.QUERY_CLOTHES = _C.DATA.ROOT_QUERY
# The number of instances per identity for training sampler
_C.DATA.NUM_INSTANCES = 8
# USE GENERATED DATA OR NOT
_C.DATA.USE_GENERATED = True
_C.DATA.START_USE_GENERATED_DATA=-1
_C.DATA.SAME_IMG = True
...
_C.MODEL.RESUME = './logs/res50-cels-cal/best_model.pth.tar'
...
_C.CL = CN()
_C.CL.CL_TYPE=['']
_C.CL.THRESHOLDS = [5, 10, 15]
_C.CL.PERCENTAGES = [0.25, 0.5, 0.75]
_C.CL.NO_CLOTHES = [1, 2, 3]
# -----------------------------------------------------------------------------
# Testing settings
# -----------------------------------------------------------------------------
_C.TEST = CN()
...
def update_config(config, args):
    ...
    if args.gen_path:
        config.DATA.ROOT_QUERY = args.gen_path
        config.DATA.QUERY_CLOTHES = args.gen_path
    if args.output:
        config.OUTPUT = args.output
