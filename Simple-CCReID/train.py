...
def train_cal_generated_data(config, epoch, model, classifier, clothes_classifier, criterion_cla, criterion_pair,
                             criterion_clothes, criterion_adv, optimizer, optimizer_cc, trainloader, pid2clothes):
    print("Train cal with generated data")
    ...
    for batch_idx, (imgs, gen_imgs, pids, camids, clothes_ids) in enumerate(trainloader):
        # Get all positive clothes classes (belonging to the same identity) for each sample
        bs = imgs.shape[0]
        if config.DATA.START_USE_GENERATED_DATA <= epoch:
            imgs = torch.cat((imgs, gen_imgs))
            pids = torch.cat((pids, pids))
            imgs = imgs[:config.DATA.MAX_BATCH_SIZE]
            pids = pids[:config.DATA.MAX_BATCH_SIZE]
        imgs, pids, clothes_ids = imgs.cuda(), pids.cuda(), clothes_ids.cuda()

        pos_mask = pid2clothes[pids]
        pos_mask = pos_mask[:bs].float().cuda()
        data_time.update(time.time() - end)
        # Forward
        features = model(imgs)
        outputs = classifier(features)
        pred_clothes = clothes_classifier(features[:bs].detach())
        ...
        # Update the backbone
        new_pred_clothes = clothes_classifier(features[:bs])
    ...
    trainloader.dataset.update_epoch_count()
    logger.info('Epoch{0} '
